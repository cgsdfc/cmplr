...
void init_float_literal(void)
{
  ...
  config_action(TKA_COLLECT_CHAR);
    ...
    config_action(TKA_ACC_FLOAT);
      config_to(fini);
        config_usrd(CC_REV);
          config_cond(Dec_suffix);
            add_from(float_suffix);
            add_from(exponent);
          config_cond(Dec_suffix_e);
            add_from(fraction);
    config_end();
}
  ...
  config_action(TKA_ACC_INT);
    config_to(fini);
      config_usrd(CC_REV);
        config_cond(suffix);
          add_from(int_suffix);
        config_cond(dec_suffix_dot_e);
          add_from(TK_INT_DEC_BEGIN);
        config_cond(oct_suffix_dot);
          add_from(oct_begin);
        config_cond(hex_suffix_dot);
          add_from(hex_begin);
        config_cond(oct_suffix_dot_x_e);
          add_from(TK_INT_ZERO);
  config_end();
}
