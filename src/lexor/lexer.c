...
typedef enum char_source { CHAR_SOURCE_FILE, CHAR_SOURCE_STRING } char_source;

static Lexer *after_create_char_buffer(char_buffer *cb) {
...
static char_buffer *before_create_char_buffer(char *data, char_source src) {
...
Lexer *CreateLexerFromFile(char *file) {
...
Lexer *CreateLexerFromString(char *str) {
...
void LexerError(Lexer *lexer) { tknzr_error_handle(lexer->cb); }
...
int LexerPrintToken(Lexer *lexer) {
  /* token *tk; */
  token *tk;
  while (true) {
    switch (get_token(lexer, &tk)) {
...
const char **LexerTerminalTab(void) { return NULL; }

const char *LexerTerminalString(Token *t) {
