...
typedef enum char_source { CHAR_SOURCE_FILE,
	    CHAR_SOURCE_STRING } char_source;
...
{
	char_buffer *cb =
	    before_create_char_buffer(file, CHAR_SOURCE_FILE);
	if (!cb)
		return NULL;
...
{
	char_buffer *cb =
	    before_create_char_buffer(str, CHAR_SOURCE_STRING);
	if (!cb)
		return NULL;
	return after_create_char_buffer(cb);
}
