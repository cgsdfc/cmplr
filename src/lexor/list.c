...
// O <- O <- O <- tail
void init_token_list(token_list * list, char_buffer * cb)
{
...
static token_list_node *make_node(token * t)
{
...
static void destroy_node(token_list_node * node)
{
	destroy_token(node->token);
	free(node->token);
}

static void push_back_node(token_list * list, token_list_node * node)
{
...
static int push_back_token(token_list * list)
{
...
int get_token(token_list * list, token ** tk)
{
	if (list->eof) {
		return EOF;
	}
