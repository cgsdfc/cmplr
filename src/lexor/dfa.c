...
void config_from(int from) { cur_config.from = from; }

void config_to(int to) { cur_config.to = to; }

void config_condition(int cond) { cur_config.cond = cond; }

void config_action(int action) { cur_config.action = action; }

void config_usrd(int usrd) { cur_config.usrd = usrd; }
...
int add_config(void) { add_state(cur_config.from, cur_config.to); }
...
// entries
int add_from(int from) { return add_state(from, cur_config.to); }

// after calling config_from,
// call add_to will add `to`
// into `from`'s entries configed
// before
int add_to(int to) { return add_state(cur_config.from, to); }
