...
static const err_entry error_tab[] = {
        [ER_NO_INPUT] = {"no input file specified, compilation stops",
                         LV_FATAL},
        ...
        [ER_DIGIT_NOT_HEXADECIMAL] = {"invalid digit in hexadicemal constant",
                                      LV_ERROR},
        [ER_BAD_INTEGER_SUFFIX] = {"invalid suffix on integer constant",
                                   LV_ERROR},
        [ER_FLOAT_EXP_NO_DIGIT] = {"exponent has no digits", LV_ERROR},
        [ER_BAD_FLOAT_SUFFIX] = {"invalid suffix on floating point constant",
                                 LV_ERROR},
        [ER_STRING_UNTERMINATED] = {"missing terminating \" character",
                                    LV_ERROR},
...
static const char *indicator[] = {[LV_FATAL] = RED_FATAL,
                                  [LV_ERROR] = RED_ERROR, [LV_NOTE] = CYAN_NOTE,
                                  [LV_WARNING] = PURPLE_WARNING};

static char *tknzr_error_string(void) {
...
void tknzr_error_set(int err) { cmplr_errno = err; }

int tknzr_error_handle(char_buffer *cb) {
  put_char(cb);
  char *errline = peek_line(cb, get_lineno(cb));
  char *errmsg = tknzr_error_string();

  fprintf(stderr, "%s:%d:%d:%s\n", cb->filename, cb->pos.lineno, cb->pos.column,
          errmsg);
