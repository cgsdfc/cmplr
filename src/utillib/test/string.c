...
UTILLIB_TEST_SET_UP() { utillib_string_init(UT_FIXTURE); }

UTILLIB_TEST_TEAR_DOWN() { utillib_string_destroy(UT_FIXTURE); }
...
UTILLIB_TEST(clear) {
  ...
  if (0 == utillib_string_capacity(UT_FIXTURE)) {
    UTILLIB_TEST_ABORT("The current implementation of string should not free "
                       "memory when doing clear!");
  }
...
UTILLIB_TEST(reserve) {
  ...
  for (int i = 0; i < 20; i++) {
    UTILLIB_TEST_ASSERT_GE(utillib_string_capacity(UT_FIXTURE),
