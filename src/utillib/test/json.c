...
UTILLIB_JSON_OBJECT_FIELD_ELEM(Student, "Name", name,
...
Student students[] = {{.name = "John", .id = 12, .gpa = 3.2},
...
static utillib_json_value_t *Point_JSON_create(void const *base,
                                               size_t offset) {
...
UTILLIB_TEST(json_object_create_nested_object) {
  ...
  utillib_json_value_t *A_val = Point_JSON_create(&A, sizeof A);
  UTILLIB_TEST_AUX_INVOKE(tostring_helper, A_val);

  Line L = {{1.0, 2.0}, {3.4, 3.2}};
  utillib_json_value_t *L_val =
 ...
 */
UTILLIB_JSON_ARRAY_DESC(Student_Desc, sizeof(Student), Student_Create);
...
UTILLIB_TEST(json_array_in_object) {
  ...
  UTILLIB_JSON_OBJECT_FIELD_ELEM(Real_Array, "array", array,
                                 utillib_json_real_array_create)
  UTILLIB_JSON_OBJECT_FIELD_ELEM(Real_Array, "size", size,
...
static utillib_json_value_t *status_string_create(void const *base,
                                                  size_t offset) {
  int status = *(int *)base;
...
UTILLIB_TEST(test_entry_create) {
  utillib_test_entry_t entry = {.func_name = "func_name",
                                .assert_failure = 1,
                                .expect_failure = 3,
                                .abort_failure = 1,
                                .succeeded = false};
