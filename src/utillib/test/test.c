...
UTILLIB_TEST(message) {
  ...
  UTILLIB_TEST_MESSAGE("A message with format `%s', `%d', `%c'", "string", 4,
                       'c');
}
...
UTILLIB_TEST(statistics_correct) {
  // sort of reflection.
  struct utillib_test_env *env = Utillib_Test();
...
UTILLIB_TEST(abort_failed) { UTILLIB_TEST_ABORT("Say, fatal error, ENOMEM!"); }

UTILLIB_TEST(should_be_skip_by_abort) {
  UTILLIB_TEST_ASSERT(false && "should not reach here");
}

UTILLIB_TEST_SET_UP() { puts("setup function called"); }

UTILLIB_TEST_TEAR_DOWN() { puts("tear down function called"); }
