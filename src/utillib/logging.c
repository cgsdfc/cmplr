...
static void utillib_logmsg_output(FILE * stream,
				  utillib_logging_msg_t * msg)
{
	char const *lv_str = logging_level_t_tostring(msg->lmsg_lv);
	fprintf(stream, "%s:%s:%s:%lu: %s\n", lv_str, msg->lmsg_file,
		msg->lmsg_func, msg->lmsg_line, msg->lmsg_msg);
}
...
static void utillib_logmsgV(utillib_logging_core_t * core, int level,
			    char const *file, char const *func,
			    size_t line, char const *fmt, va_list ap)
{
...
{
	utillib_logging_core_set_cleanup(&static_logging_core, toclean,
					 cleanup);
}
			    ...
			    size_t line, char const *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	utillib_logmsgV(&static_logging_core, level, file, func, line, fmt,
			ap);
	va_end(ap);
}
...
{
	utillib_logging_core_set_stderr_threshold(&static_logging_core,
						  level);
}
