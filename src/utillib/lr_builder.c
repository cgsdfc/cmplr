...
lr_builder_max_items(struct utillib_rule_index const *rule_index) {
  ...
  UTILLIB_VECTOR_FOREACH(rule, &rule_index->rules) {
...
static size_t lr_item_hash(struct utillib_lr_item const *self) {
  return self->pos + self->rule->id;
}

static int lr_item_compare(struct utillib_lr_item const *lhs,
...
static const struct utillib_hashmap_callback lr_item_callback = {
    .hash_handler = lr_item_hash, .compare_handler = lr_item_compare,
};

static size_t lr_itemset_hash(struct utillib_lr_itemset const *self) {
  return utillib_bitset_hash(&self->bitset);
}

static int lr_itemset_compare(struct utillib_lr_itemset const *lhs,
...
static const struct utillib_hashmap_callback lr_itemset_callback = {
    .hash_handler = lr_itemset_hash, .compare_handler = lr_itemset_compare,
};
 ...
 * any more.
 */
void utillib_lr_itemset_closure(struct utillib_lr_builder *self,
  ...
  for (int i = 0; i < utillib_vector_size(items); ++i) {
    ...
    if (!prefixed || prefixed->kind == UT_SYMBOL_TERMINAL)
      continue;
    size_t prefixed_index =
        utillib_rule_index_symbol_index(rule_index, prefixed);
    ...
    UTILLIB_VECTOR_FOREACH(rule, prefixed_rules) {
      utillib_lr_item_init(&new_item, 0, rule);
      utillib_lr_itemset_additem(
          itemset, utillib_lr_builder_item_lookup(self, &new_item));
    }
  }
}
...
static void lr_builder_core_bfs(struct utillib_lr_builder *self,
  ...
  struct utillib_vector *gotosets = &self->gotosets;
  struct utillib_lr_itemset *reachable;
  ...
  while (!utillib_list_empty(queue)) {
    ...
    UTILLIB_VECTOR_FOREACH(reachable, gotosets) {
      ...
      utillib_edge_list_add_edge(core, topset->value, reachable->value,
                                 reachable->access);
