...
{
	status_output(status_str, "%s (%lds).", entry->func_name,
		      entry->duration);
}
				 ...
				 utillib_test_env_t * env)
{
	...
	case UT_STATUS_RUN:
		...
		if (0 ==
		    self->abort_failure + self->assert_failure +
		    self->expect_failure) {
			++env->nsuccess;
			self->succeeded = true;
			/* displays a status bar */
...
static void utillib_test_setup(utillib_test_env_t * self)
{
	...
	status_output(GREEN_BANG, "Test suite `%s' sets up.",
		      self->case_name);
}
...
{
	status_output(GREEN_DASH,
		      "Summary: Total %lu tests, Run %lu, Skipped %lu.",
		      self->ntests, self->nrun, self->nskipped);
	status_output(GREEN_DASH,
		      "Summary: Run %lu tests, Passed %lu, Failed %lu.",
...
{
	status_output(GREEN_BANG, "Test suite `%s' tears down.",
		      self->case_name);
}
...
int utillib_test_run_suite(utillib_test_env_t * self)
{
	...
	for (utillib_test_entry_t * test = self->cases; test->func != NULL;
	     ++test) {
 ...
 */
static utillib_json_value_t *json_test_entry_create(void *base,
						    size_t offset)
{
...
static utillib_json_value_t *json_test_entry_array_pointer_create(void
								  *base,
								  size_t
								  offset)
{
	...
	return utillib_json_array_pointer_create(*(void **) base,
						 *(size_t *) psize,
						 &TestEntry_ArrayDesc);
}
 ...
 */
static utillib_json_value_t *json_test_env_create(void *base,
						  size_t offset)
{
...
static void test_suite_print_json(utillib_test_suite_t * self)
{
	...
	if (!file) {
		status_output(RED_DASH, "warning: `%s': %s",
