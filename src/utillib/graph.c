...
void utillib_edge_list_destroy_owning(struct utillib_edge_list *self,
  ...
  void *node;
  struct utillib_edge *edge;

  if (node_destroy) {
    UTILLIB_VECTOR_FOREACH(node, &self->nodes) { node_destroy(node); }
  }
  if (edge_destroy) {
    UTILLIB_VECTOR_FOREACH(edge, &self->edges) { edge_destroy(edge->value); }
  }
...
struct utillib_json_value *utillib_edge_list_json_object_create(
  ...
  struct utillib_edge const *edge;
  ...
  if (edge_create) {
    ...
    UTILLIB_VECTOR_FOREACH(edge, &self->edges) {
      struct utillib_json_value *edge_object =
