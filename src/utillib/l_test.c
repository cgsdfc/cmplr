...
static void report_test_status(utillib_test_env_t *self, char const *bang,
  ...
  fprintf(stderr, "%s %s.%s\n", bang, self->case_name, test->func_name);
}
...
static void print_summary(utillib_test_env_t *self) {
  ...
  UTILLIB_VECTOR_FOREACH(utillib_test_entry_t *, test, &self->cases_vec) {
    if (test->success) {
...
static void run_single_test(utillib_test_env_t *self,
                            utillib_test_entry_t *test) {
  fprintf(stderr, "%s %s.%s\n", GREEN_RUN, self->case_name, test->func_name);
...
static void register_test(utillib_test_env_t *self) {
  ...
  for (; p->func != NULL; ++p) {
    utillib_unordered_map_emplace(&self->cases_map, p->func_name, p);
  }
  self->ntests = p - self->cases;
}

void utillib_test_env_init(utillib_test_env_t *self) { register_test(self); }
...
bool utillib_test_predicate(utillib_test_entry_t *self,
  ...
  case US_ASSERT:
    ...
    break;
  case US_ABORT:
    assert(self->nabort == 0 && "A test cannot be abort twice");
    self->nabort++;
    break;
  }
