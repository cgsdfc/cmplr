...
int
init_dfa (dfa_table * dfa, int nrows, transfer_func func)
{
  if (!func)
    return -1;
  if (!dfa)
...
dfa_table *
alloc_dfa (int nrows, transfer_func func)
{
  int count = dfa_count;
...
void
config_condition (int cond)
{
  cur_config.cond = cond;
}

void
config_action (int action)
{
  cur_config.action = action;
}

void
config_usrd (int usrd)
{
...
int
alloc_state (bool is_non_terminal)
{
...
// if fail, it return 1, 
int
transfer (dfa_table * dfa, int from, int cond, dfa_state ** to)
{
