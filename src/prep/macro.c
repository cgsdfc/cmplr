...
static prep_macro_entry_t *prep_macro_entry_create(int kind,
    char const *data)
{
...
static void macro_function_init(prep_macro_funtion_t * self,
    char const *name)
{
  self->name = name;
  ...
  for (char const *str = defs; *str; ++str) {
    switch (*str) {
      case ')':
        return ++str;
      case ',':
        utillib_vector_push_back(&self->args,
            strdup
            (utillib_string_c_str
             (&buf)));
    ...
    static void
    macro_function_parse_body(prep_macro_funtion_t * self,
        char const *body) {
    }
  }
}
 ...
 */
static void macro_function_parse(prep_macro_funtion_t *
    self, char const *defs) {
  char const *body =
  macro_function_parse_args(self, defs);
  macro_function_parse_body(self, body);
}

static prep_macro_funtion_t *macro_function_create(char
    const
    *name,
    char
    const
...
prep_macro_definition_t *prep_macro_create(char const
    *name,
    char const
    *maybe_function)
{
...
prep_macro_expansion_t *
prep_macro_expansion_create(prep_macro_t *self, 
