...
{
	utillib_vector_push_back(self->psp_err,
				 scanner_input_buf_make_error(&self->
							      psp_buf,
							      ERROR_LV_ERROR,
							      msg));
}
...
void prep_scanner_init(prep_scanner_t * self, FILE * file,
		       char const *filename, utillib_vector * err)
{
