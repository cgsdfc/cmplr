...
static void push_error(prep_parser_t *self, int lv, char const *fmt, ...) {
  ...
  utillib_vector_push_back(
      &self->psp_errors,
      scanner_input_buf_make_errorV(&self->psp_cur_scan->psp_buf, lv, fmt, ap));
  va_end(ap);
}
...
void prep_parser_init(prep_parser_t *self, prep_arg_t *arg) {
  ...
  self->psp_cur_scan =
      make_scanner(arg->arg_file, arg->arg_filename, &self->psp_errors);
  ...
  UTILLIB_VECTOR_FOREACH(char const *, macro, &arg->arg_defs) {
    utillib_unordered_map_emplace(&self->psp_macro_map, macro, "1");
  }
}
...
static void push_scanner(prep_parser_t *self, prep_scanner_t *scan) {
  utillib_slist_push_front(&self->psp_incl_stack, self->psp_cur_scan);
...
static int do_include(prep_parser_t *self) {
  ...
  default:
    lv = ERROR_LV_ERROR;
    push_error(self, lv, "#include expects \"FILENAME\" or <FILENAME>");
    return lv;
  }
  ...
  push_scanner(self, make_scanner(file, filename, &self->psp_errors));
  return lv;
}
...
static size_t find_directive(prep_parser_t *self) {
  ...
  if (!(pair = utillib_unordered_map_find(&self->psp_directives, text))) {
    return PREP_ERR;
  }
...
static void do_unknown(prep_parser_t *self) {
  ...
  int ch;
  push_error(self, ERROR_LV_WARNING, "ignoring unknown directive `%s'",
             unknown);
  do {
...
static void do_undef(prep_parser_t *self) {
  ...
  case PREP_ID:
    utillib_unordered_map_erase(&self->psp_macro_map,
                                prep_scanner_get_text(self->psp_cur_scan));
    break;
  case PREP_ERR:
    break;
  default:
...
static void do_display(prep_parser_t *self, int lv, char const *errmsg) {
  ...
  default:
    // other illegal token.
    user_msg = incomplete;
    push_error(self, ERROR_LV_ERROR, "expected `\"...\"' after `%s'", errmsg);
    break;
  }
  ...
  while (true) {
    ...
    case PREP_CHAR_SHARP:
      ...
      case DR_WARNING:
        ...
        break;
      case DR_INCLUDE:
        if (ERROR_LV_FATAL == (lv = do_include(self))) {
          return lv;
        }
        break;
      case DR_DEFINE:
