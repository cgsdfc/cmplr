				   ...
				   utillib_argp_state * state)
{
	...
	static utillib_argp_error_t prep_argp_def(int argc, char **argv,
		...
		UTILLIB_ARGP_OPTION_BEGIN(prep_options)
		    UTILLIB_ARGP_OPTIOIN_ELEM(0, 'I', "DIR", 0,
					      "each -IDIR specifies a directory to search for include directive",
					      0)
		    UTILLIB_ARGP_OPTIOIN_ELEM(0, 'D', "KEY", 0,
					      "define a macro with optional "
					      "value as if it was defined by "
					      "a `#define' directive", 0)
		    UTILLIB_ARGP_OPTION_END(prep_options)
		    UTILLIB_ARGP_BEGIN(prep_argp, &prep_options, prep_parser, "[options] file...",	/* args_doc */
				       "C preprocessor",	/* doc */
				       0,	/* child */
				       0,	/* domain */
		    )
