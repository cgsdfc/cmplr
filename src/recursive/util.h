...
#define UTIL_IS_FUNC_DECLARE(PERFIX, NAME)                                     \
  int PERFIX##_is_##NAME(struct pcontext *context)

#define UTIL_PROPAGATE_RETURN(FUNC, ARG_1)                                     \
  switch (FUNC(ARG1)) {                                                        \
  case RETURN_ERROR:                                                           \
    return RETURN_ERROR;                                                       \
  case RETURN_ACCEPT:                                                          \
...
#define UTIL_PFUNC_ELEM_INIT(TAG, NAME, STRING)                                \
  [TAG] = {.apply = NAME, .name = STRING},
...
bool util_is_terminal(struct pcontext *, TokenType tt,
		      bool /* pushing */ );
bool util_is_terminal_pred(struct pcontext *, tfunction * pred,
			   bool /* pushing */ );
