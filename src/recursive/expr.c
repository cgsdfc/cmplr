...
EXPR_IS_FUNC_DECLARE(postfix_list) {
  ...
  case TKT_LEFT_PARENTHESIS:
    switch (what == TKT_LEFT_PARENTHESIS ? r = expr_is_optional_arglist(context)
  ...
  case TKT_BINARY_OP_MEMBER_ARROW:
    ...
    if (util_is_identifier(t)) {
      pcontext_reduce_binary(context, util_token_type_to_construct(what));
      return RETURN_ACCEPT;
    }
    return pcontext_push_expected_error(context, ERR_EXPECTED, "identifier",
                                        ERR_AFTER, "'->' or '.' token", NULL,
                                        NULL);
  case TKT_UNARY_OP_PLUS_PLUS:
  case TKT_UNARY_OP_MINUS_MINUS:
    util_shift_one_token(context);
    pcontext_reduce_unary(context, util_token_type_to_construct(what));
    return RETURN_ACCEPT;
  default:
    return RETURN_REJECT;
  }
...
EXPR_IS_FUNC_DECLARE(unary) {
  ...
  case TKT_UNARY_OP_MINUS_MINUS:
    ...
    case RETURN_ERROR:
      return r;
    case RETURN_ACCEPT:
      c = util_token_type_to_construct_op(TOKEN_TYPE(t), NODE_TAG_UNARY);
      pcontext_reduce_unary(context, c);
      return r;
    case RETURN_REJECT:
      return pcontext_push_expected_error(context, EXPECTED, "unary expression",
                                          AFTER, "'++' or '--' token", NULL,
                                          NULL);
    }
  case TKT_KW_SIZEOF:
    ...
    case RETURN_ACCEPT:
      return r;
    case RETURN_REJECT:
      return pcontext_push_expected_error(context, EXPECTED,
                                          "typename or unary expression", AFTER,
                                          "'sizeof' token", NULL, NULL);
    }
  default:
    return RETURN_REJECT;
  }
...
static EXPR_IS_FUNC_DECLARE(ternary_seq) {
  ...
  return util_is_sequence(context, util_is_question, expr_is_expression,
                          util_is_colon, expr_is_condition, NULL);
}
...
EXPR_IS_FUNC_DECLARE(optional_constant) {
  pcontext_set_pfunction_name(context, "optional constant expression");
