...
UTILLIB_ETAB_ELEM(NODE_TAG_DECL)
UTILLIB_ETAB_END(node_tag)
void init_node_base(node_base *base, node_tag tag) { base->tag = tag; }

node_base *make_terminal_node(Token *tk) {
...
node_base *make_binary_node(int op) {
...
node_base *make_unary_node(int op) {
...
node_base *make_vector_node() {
  CREATE_NODE(vector_node, NODE_TAG_VECTOR);
  utillib_vector_init(&(node->vec));
  return TO_NODE_BASE(node);
}

int vector_node_push_back(vector_node *node, node_base *x) {
  return utillib_vector_push_back(&(node->vec), x);
}

node_base *vector_node_at(struct vector_node *node, size_t pos) {
  return utillib_vector_at(&(node->vec), pos);
}

size_t vector_node_size(struct vector_node *node) {
...
void destroy_vector_node(vector_node *node) {
  utillib_vector_destroy(&(node->vec));
  free(node);
}

void visit_node(void *data, node_base *node, vfactory *factory) {
