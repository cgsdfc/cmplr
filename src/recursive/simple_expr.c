...
bool
success (parse_info * info)
{
  return info->res == PARSE_RESULT_SUCCESS;
}

bool
failed (parse_info * info)
{
  return info->res == PARSE_RESULT_FAILED;
}

void
set_success (parse_info * info)
{
  info->res = PARSE_RESULT_SUCCESS;
}

void
set_failed (parse_info * info)
{
  info->res = PARSE_RESULT_FAILED;
}

void
set_eofhit (parse_info * info)
{
  info->res = PARSE_RESULT_EOFHIT;
}

void
set_node (parse_info * info, void *node)
{
  info->node = node;
}

bool
check_eof (token_buffer * tokens)
{
  token *tk;
...
void
parse_success (token_buffer * tokens, parse_info * info, void *node)
{
  if (node)
    {
      token *tk;
...
void
parse_term_oper (token_buffer * tokens, parse_info * info)
{
  if (check_eof (tokens))
    {
      set_eofhit (info);
...
void
parse_term (token_buffer * tokens, parse_info * info)
{
...
void
print_number (token * tk)
{
  printf ("%s", TOKEN_STRING (tk));
}

void
print_unary_node (unary_node * unary)
{
...
void
print_binary_node (binary_node * bin)
{
