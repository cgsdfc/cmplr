...
{
	return terminal_is_identifier(t) || terminal_is_int_const(t) ||
	    terminal_is_float_const(t) || terminal_is_string_const(t);
}

TERMINAL_DECLARE_PRED(storage_specifier)
{
	return terminal_is_auto(t) || terminal_is_static(t) ||
...
{
	TokenType tt = TOKEN_TYPE(t);
	return tt == TKT_AMPERSAND || tt == TKT_STAR || tt == TKT_PLUS ||
	    tt == TKT_MINUS || tt == TKT_UNARY_OP_BIT_NOT ||
...
TERMINAL_DECLARE_PRED(assign_op)
{
	...
	return tt == TKT_BINARY_OP_ASSIGN || tt == TKT_BINARY_OP_MUL_ASSIGN
	    || tt == TKT_BINARY_OP_DIV_ASSIGN
	    || tt == TKT_BINARY_OP_ADD_ASSIGN
...
{
	TokenType tt = TOKEN_TYPE(t);
	return tt == TKT_KW_SIGNED || tt == TKT_KW_INT || tt == TKT_KW_LONG
	    || tt == TKT_KW_CHAR || tt == TKT_KW_SHORT
	    || tt == TKT_KW_UNSIGNED;
}
