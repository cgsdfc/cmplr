...
static int util_is_surrounded(pcontext * context, pfunction * parse,
			      int left, int right)
{
	if (util_is_terminal(context, left, false))	// pushing
	{
		...
		case RETURN_ACCEPT:
			break;
		case RETURN_REJECT:
			pcontext_push_expected_error(context, ERR_EXPECTED,
						     pcontext_get_pfunction_name
						     (context), ERR_AFTER,
						     util_token_type_tostring
						     (left), NULL, NULL);
			return RETURN_ERROR;
		}
...
int util_is_list(pcontext * context, pfunction * parse, size_t least)
{
	while (true) {
		...
		int util_is_in_parentheses(pcontext * context,
					   pfunction * parse) {
		...
		int util_is_in_braces(pcontext * context,
				      pfunction * parse) {
			return util_is_surrounded(context, parse,
						  TKT_LEFT_BRACE,
						  TKT_RIGHT_BRACE);
		...
		int util_is_in_brackets(pcontext * context,
					pfunction * parse) {
			return util_is_surrounded(context, parse,
						  TKT_LEFT_BRACKET,
						  TKT_RIGHT_BRACKET);
		...
		static int util_is_terminal_success(pcontext * context,
						    Token * t,
						    int pushing) {
			pcontext_shift_token(context, 1);
			if (pushing) {
				pcontext_push_node(context,
						   make_terminal_node(t));
			}
			return true;
		}
		...
		int /* NOTICE: simple predicate can use this */
		    util_is_terminal(pcontext * context,
				     TokenType expected, int pushing) {
			Token *t = util_read_first_token(context);
			if (expected == TOKEN_TYPE(t)) {
				return util_is_terminal_success(context, t,
								pushing);
			}
			return false;
		}

		int util_is_terminal_pred(struct pcontext *context,
					  tfunction * pred, int pushing) {
			Token *t = util_read_first_token(context);
			if (pred(t)) {
				return util_is_terminal_success(context, t,
								pushing);
		...
		int util_is_identifier(pcontext * context) {
			return util_is_terminal(context, TKT_IDENTIFIER,
						true);
		}

		int util_is_semicolon(pcontext * context) {
			return util_is_terminal(context, TKT_SEMICOLON,
						false);
		}

		int util_is_comma(pcontext * context) {
			// discard the colon
			return util_is_terminal(context, TKT_COMMA,
						false /* pushing */ );
		}
		...
		int util_is_separated_list_reduce_binary(pcontext *
							 context,
							 TokenType sep,
							 pfunction *
							 each) {
...
#define UTIL_TKT_TO_CTR_CASE(LABEL, RETURN)                                    \
  case (LABEL):                                                                \
    return (RETURN);
			       ...
			       static int
			       util_token_type_to_construct_impl(int tt,
								 int tag) {
			       UTIL_TKT_TO_CTR_BEGIN(tt)
case TKT_AMPERSAND:
			       UTIL_TKT_TO_CTR_BEGIN(tag)
			       UTIL_TKT_TO_CTR_CASE(NODE_TAG_UNARY,
						    OP_ADDROF)
			       UTIL_TKT_TO_CTR_CASE(NODE_TAG_BINARY,
						    OP_BIT_AND)
...
case TKT_MINUS:
			       ...
			       return util_token_type_to_construct_impl(tt,
									-1);
			       }

			       int util_token_type_to_construct_op(int tt,
								   int tag)
			       {
			       return util_token_type_to_construct_impl(tt,
									tag);
