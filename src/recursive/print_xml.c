...
static void print_xml_open_tag_impl(xml_printer * p, const char *tag,
				    va_list ap, bool strict)
{
	const char *key, *value;
	print_xml_indent(p);
	fprintf(p->file, "<%s", tag);
	for (key = va_arg(ap, const char *), value =
	     va_arg(ap, const char *); key != NULL || value != NULL;
	     key = va_arg(ap, const char *), value =
...
// print_xml_terminal
// ====================================================================== //
static void print_xml_terminal_impl(xml_printer * p,
				    terminal_node * terminal)
{
	Token *tk = terminal->token;
	print_xml_unstrict_tag(p, "terminal", "token_type",
			       (char *) print_xml_access_tag(p,
							     TOKEN_TYPE
							     (tk)),
...
{
	print_xml_ternary_impl(p, IF_NODE_BODY(if_node), "if", "condition",
			       "then", "else");
}
...
static void print_xml_default_stmt(xml_printer * p,
				   unary_node * default_node)
{
...
static void print_xml_switch_stmt(xml_printer * p,
				  binary_node * switch_node)
{
	print_xml_binary_impl(p, switch_node, "switch", "expression",
			      "statement");
}
...
static void print_xml_binary_expr(xml_printer * p, binary_node * binary)
{
	print_xml_binary_impl(p, binary,
			      print_xml_access_tag(p,
						   BINARY_NODE_OP(binary)),
...
static void print_xml_for_stmt(xml_printer * p, binary_node * for_node)
{
	...
	print_xml_ternary_impl(p, FOR_NODE_HEAD(for_node), "head",
...
{
	print_xml_binary_impl(p, do_while_node, "do-while", "stmt",
			      "expr");
}
...
static void print_xml_compound_stmt(xml_printer * p,
				    binary_node * compound)
{
	print_xml_binary_impl(p, compound, "compound", "decl_list",
			      "stmt_list");
}
...
{
	print_xml_ternary_impl(printer, node, "ternary", "first", "second",
			       "third");
}
...
static void print_xml_vector_impl(xml_printer * p, vector_node * v)
{
	...
	print_xml_open_tag_attri(p, "vector", "size", size_buf, NULL,
				 NULL);
