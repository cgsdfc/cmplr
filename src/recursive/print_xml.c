...
{
  print_xml_unary_impl(p, goto_node, 
      "goto", "label");
}
static void
print_xml_default_stmt(xml_printer *p, unary_node *default_node)
{
  print_xml_unary_impl(p, default_node,
      "default", "statement");
}

static void
print_xml_return_stmt(xml_printer *p, node_base *return_node, bool has_expr)
{
  if (has_expr) {
...
print_xml_binary_impl (xml_printer * p, binary_node* binary,
   const char *tag,const char *lhs_tag,const char *rhs_tag)
{
  print_xml_open_tag (p, tag);
