...
static void print_xml_ternary_impl (xml_printer * p,
...
void
init_xml_printer (xml_printer * p, FILE * file,
		  char *filename, const char **tag_array)
{
  p->indent = 0;
  p->file = file;
...
// ====================================================================== //
static void
print_xml_nullary (void *printer, void *node)
{
...
// ====================================================================== //
static void
print_xml_terminal_impl (xml_printer * p, terminal_node * terminal)
{
  Token *tk = terminal->token;
  print_xml_unstrict_tag (p,
			  "terminal", "token_type",
...
static void
print_xml_terminal (void *printer, void *node)
{
...
// ====================================================================== //
static void
print_xml_unary_impl (xml_printer * p, unary_node * unary, const char *tag,
		      const char *operand_tag)
{
  print_xml_open_tag (p, tag);
...
static void
print_xml_unary_expr (xml_printer * p, unary_node * unary)
{
  print_xml_unary_impl (p, unary, print_xml_access_tag (p, unary->op),
			"operand");
}

static void
print_xml_if_stmt (xml_printer * p, unary_node * if_node)
{
  print_xml_ternary_impl (p, IF_NODE_BODY (if_node),
			  "if", "condition", "then", "else");
}

static void
print_xml_goto_stmt (xml_printer * p, unary_node * goto_node)
{
  print_xml_unary_impl (p, goto_node, "goto", "label");
}

static void
print_xml_default_stmt (xml_printer * p, unary_node * default_node)
{
  print_xml_unary_impl (p, default_node, "default", "statement");
}

static void
print_xml_return_stmt (xml_printer * p, node_base * return_node,
		       bool has_expr)
{
  if (has_expr)
    {
      // if has_expr, return_node is a unary node
      print_xml_unary_impl (p, (unary_node *) return_node,
			    "return", "expression");
    }
  else
    {
...
static void
print_xml_unary (void *p, void *node)
{
...
// ====================================================================== //
static void
print_xml_binary_impl (xml_printer * p, binary_node * binary,
...
static void
print_xml_switch_stmt (xml_printer * p, binary_node * switch_node)
{
  print_xml_binary_impl (p, switch_node, "switch", "expression", "statement");
}

static void
print_xml_binary_expr (xml_printer * p, binary_node * binary)
{
...
static void
print_xml_for_stmt (xml_printer * p, binary_node * for_node)
{
  // cannot use print_xml_binary_impl here 
  // since the behaviour of print_xml_ternary_impl
  // need to be tuned.
  print_xml_open_tag (p, "for");
  print_xml_ternary_impl (p, FOR_NODE_HEAD (for_node),
			  "head", "initial", "condition", "step");
  print_xml_pair_of_tag (p, "body", FOR_NODE_BODY (for_node));
  print_xml_close_tag (p, "for");
}

static void
print_xml_do_while_stmt (xml_printer * p, binary_node * do_while_node)
{
  print_xml_binary_impl (p, do_while_node, "do-while", "stmt", "expr");
}

static void
print_xml_while_stmt (xml_printer * p, binary_node * while_node)
{
  print_xml_binary_impl (p, while_node, "while", "expr", "stmt");
}

static void
print_xml_compound_stmt (xml_printer * p, binary_node * compound)
{
...
// =============================================================== //
static void
print_xml_ternary_impl (xml_printer * p, ternary_node * ternary,
			const char *tag,
			const char *first_tag,
			const char *second_tag, const char *third_tag)
{
  print_xml_open_tag (p, tag);
...
static void
print_xml_ternary (void *printer, void *node)
{
  print_xml_ternary_impl (printer, node,
...
// ============================================================== //
static void
print_xml_vector_impl (xml_printer * p, vector_node * v)
{
...
static void
print_xml_vector (void *printer, void *v)
{
...
// =============================================================== //
static vfunction *
print_xml_factory (void *data, node_tag tag)
{
...
static void
print_xml_impl (void *printer, void *node)
{
  visit_node (printer, node, print_xml_factory);
}

static void
print_xml_comment (xml_printer * p)
{
...
void
print_xml (struct xml_printer *p, node_base * node)
{
  print_xml_comment (p);
  print_xml_impl (p, node);
}
