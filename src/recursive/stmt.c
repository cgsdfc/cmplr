...
STMT_IS_FUNC_DECLARE(expr_for_seq) {
  ...
  return util_is_sequence(context, expr_is_optional_expr, util_is_semicolon,
...
STMT_IS_FUNC_DECLARE(expr_stmt_seq) {
  // '(' expr ')' stmt
  return util_is_sequence(context, expr_is_in_parenthesis, stmt_is_statement,
                          NULL);
}
...
STMT_IS_FUNC_DECLARE(compound_seq) {
  return util_is_sequence(context, stmt_is_statement_list, decl_is_declare_list,
                          NULL);
}
...
STMT_IS_FUNC_DECLARE(compound) { return stmt_is_compound_impl2(context); }

STMT_IS_FUNC_DECLARE(statement) {
  return util_is_one_of(context, stmt_is_iterate, stmt_is_select, stmt_is_jump,
                        stmt_is_label, stmt_is_compound,
