...
DECL_IS_FUNC_DECLARE (type_specifier)
{
  ...
  switch (TOKEN_TYPE (t))
      ...
      if (decl_is_struct_or_union_specifier (context))
	{
      ...
      die
	("type_specifier: expected struct_or_union_specifier after 'struct_or_union' token");
    default:
...
{
  return util_is_terminal_pred (context, terminal_is_type_qualifier, true	// pushing
    );
}
...
DECL_IS_FUNC_DECLARE (specifier_qualifier)
{
  return util_is_one_of (context,
			 ...
			 decl_is_type_qualifier, NULL);
}

DECL_IS_FUNC_DECLARE (specifier_qualifier_list)
{
  return util_is_list (context, decl_is_specifier_qualifier, false	/* allow_empty */
    );
}
...
// =============================================================== //
enum
{ DSP_TPSP, DSP_TPQR, DSP_STORAGE, DSP_N };
static void
