...
//============================================================ //
static void reduce_enum_struct_union(pcontext *context, TokenType what,
...
static DECL_IS_FUNC_DECLARE(struct_decltor) {
...
static DECL_IS_FUNC_DECLARE(struct_decltor_list) {
  return util_is_separated_list(context, TKT_COMMA, decl_is_struct_decltor,
                                true /* allow_empty */);
}

static DECL_IS_FUNC_DECLARE(struct_declare) {
  ...
  if (decl_is_specifier_qualifier_list(context) &&
    ...
    binary_node *struct_declare =
        (binary_node *)make_binary_node(TKT_KW_STRUCT);
...
static DECL_IS_FUNC_DECLARE(struct_declare_list) {
  return util_is_list(context, decl_is_struct_declare, true /* allow_empty */);
}

static DECL_IS_FUNC_DECLARE(struct_or_union) {
...
DECL_IS_FUNC_DECLARE(struct_or_union_specifier) {
  ...
  case TKT_IDENTIFIER:
    ...
    if (!terminal_is_braceL(t)) { // not a body
      reduce_enum_struct_union(context, what, pcontext_pop_node(context)
                               /* tag */
                               ,
  ...
  case TKT_LEFT_BRACE:
    ...
    die("struct_or_union_specifier: expected struct_declare_list after '{' "
        "token");
  default:
    return false;
  }
...
static DECL_IS_FUNC_DECLARE(enumerator) {
...
static DECL_IS_FUNC_DECLARE(enumerator_list) {
  util_shift_one_token(context);
  if (util_is_separated_list(context, TKT_COMMA, decl_is_enumerator,
                             false /* allow_empty */)) {
...
DECL_IS_FUNC_DECLARE(enum_specifier) {
  ...
  case TKT_IDENTIFIER:
    ...
    if (!util_is_braceL(context)) {
      reduce_enum_struct_union(context, TKT_KW_ENUM,       /* what */
