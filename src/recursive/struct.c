...
DECL_IS_FUNC_DECLARE (struct_or_union_specifier)
{
  ...
  switch (TOKEN_TYPE (t))
      ...
      if (decl_is_struct_declare_list (context))
	{
	  if (util_is_braceR (context))
	    {
	      ...
	      tag =
		has_tag ? pcontext_pop_node (context) : make_nullary_node ();
      ...
      die
	("struct_or_union_specifier: expected struct_declare_list after '{' token");
    default:
      return false;
    }
}
  ...
  if (util_is_identifier (context))
      ...
      if (decl_is_assign (context))
	{
	  if (expr_is_constant (context))
	    ...
	    {
	      die
		("enumerator: expected constant expression after '=' token");
	    }
	}
      else
	{
...
DECL_IS_FUNC_DECLARE (enumerator_list)
{
  ...
  if (util_is_separated_list (context, TKT_COMMA,
			      decl_is_enumerator, false /* allow_empty */ ))
    {
