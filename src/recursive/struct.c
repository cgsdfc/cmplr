...
{
	return util_is_separated_list(context, TKT_COMMA,
				      decl_is_struct_decltor,
				      true /* allow_empty */ );
}
	...
	case TKT_LEFT_BRACE:
		...
		if (decl_is_struct_declare_list(context)) {
			if (util_is_braceR(context)) {
				...
				tag =
				    has_tag ? pcontext_pop_node(context) :
				    make_nullary_node();
				reduce_enum_struct_union(context, what,
							 tag, body);
				return true;
...
DECL_IS_FUNC_DECLARE(enum_specifier)
{
	...
	case TKT_IDENTIFIER:
		...
		if (decl_is_enumerator_list(context)) {
			...
			reduce_enum_struct_union(context, TKT_KW_ENUM, tag,
						 body);
			return true;
		}
