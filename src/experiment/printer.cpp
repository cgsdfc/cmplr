...
#include <iterator>
#define PRINTER_GET_SYMBOL(g) \
  [&](symbol_unique_id id) { return g.get_symbol(id); }
namespace experiment {
printer print(std::cout);
void printer::operator()(const_language lang) {}
void printer::flush_to_stream(string_vector_type const& toprint, bool newline,
                              const char* sep) {
...
void printer::operator()(const_grammar g, const_item item, bool newline) {
  ...
  make_string_vector(body.begin(), body.end(), toprint, PRINTER_GET_SYMBOL(g));
...
void printer::operator()(const_grammar g, const_itemset itemset, bool newline) {
  ...
  std::for_each(itemset.begin(), itemset.end(),
                [&](item_unique_id id) { operator()(g, g.item(id)); });
  if (newline) m_os << "\n";
}
void printer::operator()(const_grammar g, nonterminal2rule_map_tag) {
  boost::counting_iterator<std::size_t> begin(0),
      end(g.m_nonterminal2rule.size());
  for (; begin != end; ++begin) {
