...
namespace experiment {
  ...
  void grammar_base::resolve_symbols(const language& lang) {
    for (auto bundle:lang) {
      ...
      if (bundle.second.size()==0)
        head.set(symbol_property::optional);
  ...
  void grammar_base::sanity_check() const {
    if (m_start_symbol_id == symbol_unique_map::npos)
      throw no_start_symbol();
    if (m_eof_symbol_id == symbol_unique_map::npos)
      throw no_eof_symbol();
    if (m_nonterminal2rule[m_start_symbol_id].size()!=1)
      throw invalid_principle_rule();
  }

  const grammar_base& grammar_base::print(std::ostream& os, rule_iterator it)const {
    os << boost::format("%1% -> ") % it->first; // rule.head
    std::for_each( // rule.body
        std::begin(it->second),
        std::end(it->second),
        [&](symbol_unique_id id) { os << id << ", ";}
    );
    os << '\n';
    return *this;
  }
