...
class symbol {
  ...
  public:
    symbol(const char *str)
      : m_string(str), m_is_terminal(false) {}
    ...
    struct equal_to
      : public std::equal_to<std::string> {};
...
class rule_group {
  private:
    ...
    void add_rule(vector<symbol> && list) {
      m_body.push_back(list);
    }
    symbol m_head;
    vector<vector<symbol>> m_body;
  public:
    rule_group(const char *head)
      : m_head(head,false), m_body() {}
...
class language {
  private:
    typedef std::unordered_set<symbol,symbol::hash,symbol::equal_to>
      symbol_set;
...
terminal_adder language::terminals(){
  return terminal_adder(*this);
}
