    ...
    namespace detail { 
      ...
      template<int Cur, int End, typename... Elements>
        struct nth_element<Cur, End, vector_impl<Elements...>> {
          typedef typename
            nth_element<Cur+1, End, vector_impl<Elements...>::tail::head
          type;
        };
      ...
      template<int Cur, int End, typename... Elements>
        struct nth_element<End, End, vector_impl<Elements...>> {
          typedef typename
            vector_impl<Elements...>::head type;
        };
      ...
      struct vector {
        ...
        template <int Index>
          struct at {
            typedef typename
              detail::vector_element<Index,Elements...>::type type;
          };
...
int main (int argc, char **argv) {
  ...
  BOOST_STATIC_ASSERT (boost::is_same<
      typename vec2::template at<0>::type, 
      int>::value);

  BOOST_STATIC_ASSERT (is_same<
      typename vec2::template at<1>::type,
      double>::value);

  BOOST_STATIC_ASSERT (is_same<
      typename vec2::front::type,
      int>::value);

  BOOST_STATIC_ASSERT (is_same<
      typename vec2::back::type,
      double>::value);

  BOOST_STATIC_ASSERT (vec2::size == 2);

  typedef typename vec2::template push_back<float>::type
    vec3;

  BOOST_STATIC_ASSERT(is_same<
      typename vec3::back::type,
      float>::value);
}
