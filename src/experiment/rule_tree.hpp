...
class rule_adder {
 ...
 protected:
  ...
  rule_unique_id register_rule(symbol_unique_id head, InputIter first,
                               InputIter last) {
    rule_type rule;
    rule.head() = head;
  ...
  rule_unique_id register_rule(symbol_unique_id head,
                               symbol_id_vector const& body);
};
...
class rule_tree : public rule_adder {
 private:
  ...
  typedef rule_adder base_t;
  typedef std::vector<std::shared_ptr<rule_node>> rule_node_vector;
