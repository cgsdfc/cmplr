...
class id_already_exists : public virtual std::exception {
 public:
  const char* what() const noexcept override {
    return "pass an already existing object to make_id causes "
           "<id_already_exists> to throw";
  }
};
...
class unique_id {
 public:
  ...
  integer_type id() const { return m_id; }
  reference content() { return *m_content; }
  bool operator==(unique_id const& rhs) const { return id() == rhs.id(); }
  bool operator!=(unique_id const& rhs) const { return !(*this == rhs); }
 ...
 private:
  ...
  static integer_type current_id() { return s_id_keeper.size(); }
...
namespace std {
template <class T>
struct hash<unique_id<T>> : public hash<typename unique_id<T>::integer_type> {};
};

template <class T>
typename unique_id<T>::id_keeper_type unique_id<T>::s_id_keeper;
