...
class node_base {
  public:
    ...
    template<class T>
      using share_handle_type = std::shared_ptr<T>;
};
...
class function_name : public node_base{
  ...
  public:
    ...
    template<class... Args>
      argument_list operator(Args&&... args);
    template<class... Args>
      argument_list operator(name_type arg, Args&&... args) {
        m_args=std::make_shared<argument_list> (0); // means the argument_list is not empty
...
class argument_list : public node_base {
  private:
    ...
    share_handle_type<std::vector<share_handle_type<node_base>>>
      m_list;
  public:
