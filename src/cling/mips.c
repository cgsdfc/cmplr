 ...
 */
static const struct mips_ir mips_nop = {.opcode = MIPS_NOP};
static const struct mips_ir mips_syscall = {.opcode = MIPS_SYSCALL};

static struct mips_data *mips_string_create(char const *label,
                char const *string);
static char *mips_global_label(struct mips_global *self,
                uint32_t address);

static void mips_function_saved_push_back(struct mips_function *self,
                uint8_t regid, uint32_t offset);
static int mips_regalloc(struct mips_function *self,
                bool (*regkind)(uint8_t regid));

static void mips_para_context(struct mips_function *self, int flag);

static void mips_function_load_paras(struct mips_function *self, int index, bool is_load);
/*
 * opcode and register strings.
 */
UTILLIB_ETAB_BEGIN(mips_opcode_kind)
...
UTILLIB_ETAB_END(mips_opcode_kind);

UTILLIB_ETAB_BEGIN(mips_regster)
...
UTILLIB_ETAB_END(mips_regster);

UTILLIB_ETAB_BEGIN(mips_data_kind)
...
UTILLIB_ETAB_END(mips_data_kind);
 ...
 * to C routines.
 */
static struct mips_ir *mips_ir_create(uint8_t opcode) {
        struct mips_ir *self = calloc(1, sizeof *self);
        self->opcode = opcode;
        return self;
}
 ...
 */
static struct mips_ir *mips_jal(char const *label) {
        struct mips_ir *self = mips_ir_create(MIPS_JAL);
 ...
 */
static struct mips_ir *mips_jr(uint8_t target_regid) {
        struct mips_ir *self = mips_ir_create(MIPS_JR);
 ...
 */
static struct mips_ir *mips_j(uint32_t address) {
        struct mips_ir *self = mips_ir_create(MIPS_J);
 ...
 */
static struct mips_ir *mips_load_store(int opcode, uint8_t dest_regid,
                int16_t offset,
                uint8_t base_regid) {
        struct mips_ir *self = mips_ir_create(opcode);
 ...
 * lw $s1, -2($sp)
 */
static struct mips_ir *mips_lw(uint8_t dest_regid, int16_t offset,
                uint8_t base_regid) {
        return mips_load_store(MIPS_LW, dest_regid, offset, base_regid);
}

static struct mips_ir *mips_lb(uint8_t dest_regid, int16_t offset,
                uint8_t base_regid) {
 ...
 * sw $s2, 0($sp)
 */
static struct mips_ir *mips_sw(uint8_t src_regid, int16_t offset,
                uint8_t base_regid) {
 ...
 * sb $s2, -4($s1)
 */
static struct mips_ir *mips_sb(uint8_t src_regid, int16_t offset,
                uint8_t base_regid) {
 ...
 */
static struct mips_ir *mips_immecalc(uint8_t opcode, uint8_t dest_regid,
 ...
 */
static struct mips_ir *mips_addi(uint8_t dest_regid, uint8_t src_regid,
                int16_t imme) {
 ...
 */
static struct mips_ir *mips_regcalc(uint8_t opcode, uint8_t dest_regid,
                uint8_t src_regid1,
                uint8_t src_regid2) {
        struct mips_ir *self = mips_ir_create(opcode);
        self->operands[0].regid = dest_regid;
 ...
 * subu $s0, $s1, $s2
 */
static struct mips_ir *mips_subu(uint8_t dest_regid, uint8_t src_regid1,
                uint8_t src_regid2) {
 ...
 * sub $s0, $s1, $s2
 */
static struct mips_ir *mips_sub(uint8_t dest_regid, uint8_t src_regid1,
                uint8_t src_regid2) {
        return mips_regcalc(MIPS_SUB, dest_regid, src_regid1, src_regid2);
}

static struct mips_ir *mips_addu(uint8_t dest_regid, uint8_t src_regid1,
                uint8_t src_regid2) {
        return mips_regcalc(MIPS_ADDU, dest_regid, src_regid1, src_regid2);
}

static struct mips_ir *mips_add(uint8_t dest_regid, uint8_t src_regid1,
                uint8_t src_regid2) {
 ...
 */
static struct mips_ir *mips_la(uint8_t dest_regid, char const *label) {
        struct mips_ir *self = mips_ir_create(MIPS_LA);
        self->operands[0].regid = dest_regid;
 ...
 */
static struct mips_ir *mips_li(uint8_t dest_regid, int32_t imme32) {
        struct mips_ir *self = mips_ir_create(MIPS_LI);
        self->operands[0].regid = dest_regid;
 ...
 */
static struct mips_ir *mips_move(uint8_t dest_regid, uint8_t src_regid) {
        struct mips_ir *self = mips_ir_create(MIPS_MOVE);
        self->operands[0].regid = dest_regid;
        self->operands[1].regid = src_regid;
        return self;
}
 ...
 * Moves $lo to $s0.
 */
static struct mips_ir *mips_mflo(uint8_t dest_regid) {
        struct mips_ir *self = mips_ir_create(MIPS_MFLO);
        self->operands[0].regid = dest_regid;
        return self;
}
 ...
 */
static struct mips_ir *mips_branch(uint8_t opcode, uint8_t src_regid1,
...
static struct mips_ir *mips_beq(uint8_t src_regid1, uint8_t src_regid2,
                int16_t offset) {
 ...
 */
static struct mips_ir *mips_bne(uint8_t src_regid1, uint8_t src_regid2,
                int16_t offset) {
 ...
 */
static struct mips_ir *mips_tworegs(int opcode, uint8_t src_regid1,
                uint8_t src_regid2) {
        struct mips_ir *self = mips_ir_create(opcode);
        self->operands[0].regid = src_regid1;
        self->operands[1].regid = src_regid2;
 ...
 * and remainder in $hi.
 */
static struct mips_ir *mips_div(uint8_t src_regid1, uint8_t src_regid2) {
 ...
 */
static struct mips_ir *mips_mult(uint8_t src_regid1, uint8_t src_regid2) {
        return mips_tworegs(MIPS_MULT, src_regid1, src_regid2);
}

static void mips_ir_destroy(struct mips_ir *self) {
        switch (self->opcode) {
                case MIPS_NOP:
                case MIPS_SYSCALL:
                        /*
...
#define mips_operand_regstr(self, index)                                       \
        mips_regster_tostring(self->operands[index].regid)
void mips_ir_fprint(struct mips_ir const *self, FILE *file) 
{
        char const *opstr = mips_opcode_kind_tostring(self->opcode);
...
static void mips_function_init(struct mips_function *self,
        ...
        utillib_hashmap_init(&self->names, &string_hash);
        utillib_vector_init(&self->saved);
}

static void mips_function_destroy(struct mips_function *self) {
        free(self->reg_used);
 ...
 */
static bool mips_function_is_leaf(struct ast_function const *ast_func) {
        struct ast_ir const *ast_ir;
 ...
 */
static void mips_function_fix_address(struct mips_function *self)
{
        struct mips_ir *mips_ir;
...
static void mips_function_push_back(struct mips_function *self,
                struct mips_ir const *ir) {
        utillib_vector_push_back(self->instrs, ir);
}

static void mips_context_switch(struct mips_function *self, int flag) {

        struct mips_name const *saved;
 ...
 */
static void mips_function_prologue(struct mips_function *self) {
 ...
 * addi $sp, $sp, 128
 */
static void mips_function_epilogue(struct mips_function *self) {
 ...
 */
static void mips_temp_context(struct mips_function *self, int flag)
{
        struct mips_temp *temp_one;
 ...
 */
static uint32_t mips_alloc(struct mips_function *self, unsigned int size)
{
 ...
 */
static uint32_t
mips_function_max_args(struct ast_function const *ast_func) {
        struct ast_ir const *ast_ir;
        uint32_t arg_size;
        uint32_t max_args = 0;
...
static void mips_function_memmap(struct mips_function *self,
                char const *name, uint32_t offset) {
        struct mips_name *entry;
...
static void mips_function_temp_layout(struct mips_function *self,
                struct ast_function const *ast_func) 
{
        struct mips_name *var;
        struct ast_ir const *ast_ir;
...
static void
mips_function_local_layout(struct mips_function *self,
                struct ast_function const *ast_func) {
        struct ast_ir const *ast_ir;
...
static void mips_function_regmap(struct mips_function *self,
...
static void
mips_function_save_registers(struct mips_function *self,
                struct ast_function const *ast_func) {
        struct ast_ir const *ast_ir;
...
static void mips_function_saved_push_back(struct mips_function *self,
                uint8_t regid, uint32_t offset) {
        struct mips_name *name=malloc(sizeof *name);
        name->regid=regid;
        name->offset=offset;
        utillib_vector_push_back(&self->saved, name);
}

static void mips_function_save_ra(struct mips_function *self) {
        /*
         * Save $ra.
 ...
 */
inline static void
mips_function_args_layout(struct mips_function *self,
                struct ast_function const *ast_func) {
 ...
 */
static void
mips_function_para_layout(struct mips_function *self,
                struct ast_function const *ast_func) {
        char const *name;
        struct ast_ir const *ast_ir;
 ...
 */
static void
mips_function_stack_layout(struct mips_function *self,
                struct ast_function const *ast_func) {
        bool is_leaf;
...
static uint8_t mips_function_temp_alloc(struct mips_function *self) {
        uint8_t regid;
        uint32_t offset;
...
static uint8_t mips_function_temp_map(struct mips_function *self,
                uint16_t temp, int state , int flag) 
{
        struct mips_temp *temp_var;
        uint8_t regid;
...
static inline uint8_t mips_function_read(struct mips_function *self, int temp) {
        return mips_function_temp_map(self, temp, MIPS_HAS_REG, MIPS_READ);
}

static inline uint8_t mips_function_locked_read(struct mips_function *self, int temp) {
        return mips_function_temp_map(self, temp, MIPS_LOCKED, MIPS_READ);
}

static inline uint8_t mips_function_write(struct mips_function *self, int temp) {
 ...
 */
static void mips_temp_unlock(struct mips_function *self, int temp) {
        struct mips_temp *temp_var;
        temp_var=&self->temps[temp];
 ...
 */
static void mips_emit_ternary_calc(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t dest_regid;
 ...
 */
static void mips_emit_binary_calc(struct mips_function *self,
                struct ast_ir const *ast_ir) {
 ...
 */
static void mips_emit_index(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t index_regid;
        uint8_t array_regid;
        uint8_t dest_regid;
 ...
 * are not pooled.
 */
static char const *mips_global_asciiz(struct mips_global *self,
                char const *string) {
...
static void mips_emit_ldstr(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t regid;
        char const *label;
...
static void mips_emit_ldimm(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t dest_regid;
        size_t size;
 ...
 */
static struct mips_ir *mips_branch_relop(int relop, uint8_t regid,
                uint16_t offset) {
        struct mips_ir *ir;
 ...
 */
static void mips_emit_branch_relop(struct mips_function *self,
                struct ast_ir const *relop,
                struct ast_ir const *ast_ir) {
        assert(relop);
 ...
 */
static void mips_emit_branch(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t src_regid1, src_regid2;
...
static void mips_emit_ldnam(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        struct mips_name *named_var;
...
static void mips_emit_ldadr(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        struct mips_name *name;
...
static void mips_emit_deref(struct mips_function *self,
                struct ast_ir const *ast_ir) {
...
static void mips_emit_stadr(struct mips_function *self,
                struct ast_ir const *ast_ir) {
...
static void mips_emit_stnam(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t regid=mips_function_read(self, ast_ir->stnam.value);
        struct mips_name *name=utillib_hashmap_find(&self->names, ast_ir->stnam.name);
        assert(name);
 ...
 */
static void mips_emit_jmp(struct mips_function *self,
                struct ast_ir const *ast_ir) {
 ...
 * syscall.
 */
static void mips_emit_read(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t regid;
        int syscall;

        syscall = ast_opcode_to_syscall(ast_ir->read.kind);
        mips_function_push_back(self, mips_li(MIPS_V0, syscall));
        mips_function_push_back(self, &mips_syscall);
 ...
 */
static void mips_emit_write(struct mips_function *self,
                struct ast_ir const *ast_ir) {
        uint8_t regid;
        int syscall;
        if (self->para_size) {
         ...
         * syscall number.
         */
        syscall = ast_opcode_to_syscall(ast_ir->write.kind);
        mips_function_push_back(self, mips_li(MIPS_V0, syscall));
        mips_function_push_back(self, &mips_syscall);
        if (self->para_size) {
                /*
 ...
 * after call.
 */
static int mips_emit_call(struct mips_function *self, 
                struct ast_ir const *ast_ir) {
        uint8_t regid;
        uint32_t offset;
        struct mips_ir *ir;
 ...
 */
static void mips_function_instrs(struct mips_function *self,
                struct ast_function const *ast_func) {
        size_t ast_instrs_size;
        struct ast_ir const *ast_ir;
        struct ast_ir const *relop = NULL;
 ...
 */
static void mips_function_emit(struct mips_function *self,
                struct ast_function const *ast_func) {
 ...
 * Data Directive
 */
static struct mips_data *mips_data_create(uint8_t type,
                char const *label) {
        struct mips_data *self = malloc(sizeof *self);
        self->type = type;
        self->label = strdup(label);
        return self;
}

static struct mips_data *mips_array_create(char const *label,
                size_t extend) {
        struct mips_data *self = mips_data_create(MIPS_SPACE, label);
        self->extend = extend;
        return self;
}
...
static struct mips_data *mips_string_create(char const *label,
                char const *string) {
        struct mips_data *self = mips_data_create(MIPS_ASCIIZ, label);
        self->string = string;
        return self;
}

static void mips_data_destroy(struct mips_data *self) {
        free(self->label);
        free(self);
}

static void mips_data_print(struct mips_data const *self, FILE *file) {
        fprintf(file, "%s: %s ", self->label,
                        mips_data_kind_tostring(self->type));
        switch (self->type) {
                case MIPS_ASCIIZ:
                        fprintf(file, "\"%s\"\n", self->string);
                        break;
...
static void mips_program_emit_data(struct mips_program *self,
                struct ast_program const *program) {
        struct ast_ir const *ast_ir;
        struct mips_data *data;
 ...
 */
static void mips_program_newline(struct mips_program *self) {
        struct mips_label *label;
        ...
        utillib_vector_push_back(text, &mips_syscall);
 ...
 * and then exit.
 */
static void mips_program_setup(struct mips_program *self) {
        struct utillib_vector *text = &self->text;
...
static void mips_global_init(struct mips_global *self,
                struct mips_program *program) {
        self->label_count = 0;
...
void mips_program_print(struct mips_program const *self,
                FILE *file) {
        struct mips_data const *data;
        struct mips_ir const *ir;
        struct mips_label const *label;
        uint32_t address = 0;
...
void mips_function_layout_print(struct mips_function const *self) {
        char const *name;
        struct mips_name *entry;
        struct mips_temp *temp;
                ...
                switch(entry->kind) {
                        case MIPS_SAVED:
                                printf("%s saved at %s, offset is %u\n", name,
                                                mips_regster_tostring(entry->regid), entry->offset);
                                break;
                        case MIPS_ARGMEM:
                                printf("%s argmem\n", name);
                                break;
                        case MIPS_ARGREG:
                                printf("%s argreg at %s\n", name, 
                                                mips_regster_tostring(entry->regid));
                                break;
                        case MIPS_UNSAVED:
                ...
                switch(temp->kind) {
                        case MIPS_SAVED:
                                printf("t%d alias to saved %s\n", i, 
                                                mips_regster_tostring(temp->regid));
                                break;
                        case MIPS_ARGMEM:
