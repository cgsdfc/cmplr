 ...
 * cling_mips_interp
 */
#define MIPS_MEM_INIT_SIZE 1024

static inline size_t mips_program_size(struct cling_mips_program const *self) {
  return utillib_vector_size(&self->text);
}

static inline struct cling_mips_ir *mips_program_instr(struct cling_mips_program const *self, size_t index) {
  return utillib_vector_at(&self->text, index);
}

static void mips_interp_regdump(struct cling_mips_interp const *self, FILE *file) {
  for (int i=0; i<CLING_MIPS_REG_MAX; ++i) {
    fprintf(file, "\t%8X", self->regs[i]);
    if (i & 7 == 0)
      fputs("\n", file);
  }
  fputs("\n", file);
...
static void mips_interp_memdump(struct cling_mips_interp const *self, FILE *file) {
  for (int i=0; i<self->mem_size; ++i) {
    fprintf(file, "%4d\t%2X", self->memory[i]);
    if (i & 3 == 0)
      fputs("\n", file);
  }
  fputs("\n", file);
...
static uint32_t mips_interp_alloc(struct cling_mips_interp *self, size_t size, bool clear) {
  ...
  if (memblk > self->mem_capacity) {
    ...
    new_capacity=new_capacity>memblk?new_capacity:memblk;
    self->memory=realloc(self->memory, new_capacity * sizeof self->memory[0]);
    self->mem_capacity=new_capacity;
  }
  if (clear) {
    memset(self->memory+alloc, 0, size * sizeof self->memory[0]);
  }
  self->mem_size=memblk;
  return alloc;
}

static void mips_interp_free(struct cling_mips_interp *self, size_t size) {



}

static void mips_interp_load(struct cling_mips_interp *self,
    struct cling_mips_program const *program)
{
  ...
  /*
   * load name and address of functions.
   */
  for (int i=0; i<program->func_size; ++i) {
    range=&program->func_range[i];
    label=mips_label_create(range->label, range->begin);
    ...
    struct cling_mips_program const *program, bool clear)
{
  ...
  self->regs[MIPS_SP]=self->mem_size;
}
...
static void mips_do_move(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t dest_regid=ir->operands[0].regid;
  uint8_t src_regid=ir->operands[1].regid;
  self->regs[dest_regid]=self->regs[src_regid];
}

static int mips_do_addi(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t dest_regid=ir->operands[0].regid;
  uint8_t src_regid=ir->operands[1].regid;
...
static void mips_do_mflo(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t dest_regid=ir->operands[0].regid;
...
static void mips_do_div(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
...
static int mips_do_beq(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t src_regid1=ir->operands[0].regid;
  uint8_t src_regid2=ir->operands[1].regid;
...
static void mips_do_li(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t dest_regid=ir->operands[0].regid;
  self->regs[dest_regid]=ir->operands[1].imme32;
}

static void mips_do_jr(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t src_regid=ir->operands[0].regid;
  self->pc=self->regs[src_regid];
}

static uint32_t mips_do_address(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint8_t base_regid=ir->operands[2].regid;
  int16_t offset=ir->operands[1].offset;
  return self->regs[base_regid]+offset;
}

static int mips_do_lw(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
...
static int mips_do_sw(struct cling_mips_interp *self, struct cling_mips_ir const *ir) {
  uint32_t address=mips_do_address(self, ir);
  uint8_t dest_regid=ir->operands[0].regid;
  if (address & (MIPS_WORD_SIZE-1))
    return MIPS_EC_ALIGN;
