...
UTILLIB_ETAB_END(type);

static void symbol_entry_destroy(struct symbol_entry *self);
...
{
        self->scope = 0;
        utillib_hashmap_init(&self->global, &string_hash);
}
...
{
        utillib_hashmap_init(&self->local, &string_hash);
        ++self->scope;
}
...
static struct symbol_entry * symbol_table_lexical_find(struct symbol_table const *self, char const *name) 
{
...
static bool symbol_table_lexical_exist_name(struct symbol_table const *self, char const *name) 
{
...
void symbol_table_reserve(struct symbol_table *self, char const *name, int scope_kind) 
{
        struct utillib_hashmap *scope;

        if (scope_kind == CL_LOCAL)
                scope = &self->local;
        else if (scope_kind == CL_GLOBAL)
                scope = &self->global;
        else
                assert(false);
...
struct symbol_entry * symbol_table_find(struct symbol_table const *self, char const *name, int scope_kind) 
{
        if (!self->scope || scope_kind == CL_GLOBAL)
                return utillib_hashmap_find(&self->global, name);
        if (scope_kind == CL_LEXICAL)
                return symbol_table_lexical_find(self, name);
        else if (scope_kind == CL_LOCAL)
                return utillib_hashmap_find(&self->local, name);
        assert(false);
}

bool symbol_table_exist_name(struct symbol_table const *self, char const *name, int scope_kind) 
{
        if (!self->scope || scope_kind == CL_GLOBAL)
                return utillib_hashmap_exist_key(&self->global, name);
        switch (scope_kind) {
...
static void symbol_entry_init(struct symbol_entry *self, int scope, int kind) 
{
        self->kind = kind;
        self->scope = scope;
}

static void symbol_entry_init_single_var(struct symbol_entry *self, int scope, int symbol) 
{
        symbol_entry_init(self, scope, symbol_to_type(symbol));
}

static void symbol_entry_init_const(struct symbol_entry *self, int scope, int symbol, char const *val) 
{
        symbol_entry_init(self, scope, CL_CONST);
        self->constant.type = symbol_to_type(symbol);
        self->constant.value = symbol_to_immediate(symbol, val);
}

static void symbol_entry_init_array(struct symbol_entry *self, int scope, int base_symbol, char const *extend) 
{
        symbol_entry_init(self, scope, CL_ARRAY);
        self->array.base_type = symbol_to_type(base_symbol);
        sscanf(extend, "%lu", &self->array.extend);
}

static void symbol_entry_init_function(struct symbol_entry *self, int return_symbol, 
                                       struct utillib_json_value const *arglist) 
{
        ...
        self->function.return_type = symbol_to_type(return_symbol);
        ...
        UTILLIB_JSON_ARRAY_FOREACH(object, arglist) {
                ...
                argv_types[i++] = symbol_to_type(type->as_size_t);
        }
...
static void symbol_entry_destroy(struct symbol_entry *self) 
{
        if (!self)
                /*
                 * Since symbol_table_reserve open the window
...
static struct utillib_hashmap *current_scope(struct symbol_table *self) 
{
...
void symbol_table_insert_const(struct symbol_table *self, struct utillib_json_value const *const_decl) 
{
        struct symbol_entry *entry;
...
void symbol_table_insert_arglist(struct symbol_table *self, struct utillib_json_value const *arglist) 
{
...
void symbol_table_insert_function(struct symbol_table *self, struct utillib_json_value const *function) 
{
...
void symbol_table_insert_variable(struct symbol_table *self, struct utillib_json_value const *variable) 
{
