...
stack_buffer *alloc_stack_buffer(int elesz) {
  ...
  if (!stack_entry_buffer) {
    stack_entry_buffer = alloc_block_buffer(
        STACK_BUFFER_BLKSZ, STACK_BUFFER_INIT_NBLK, sizeof(stack_buffer_entry));
    if (!stack_entry_buffer)
      return NULL;
  }
  ...
  blk_buf = alloc_block_buffer(STACK_BUFFER_BLKSZ, STACK_BUFFER_INIT_NBLK,
                               sizeof(elesz));
  if (!blk_buf)
    return NULL;
  sb->blk_buf = blk_buf;
...
int stack_buffer_alloc(stack_buffer *buf, stack_buffer_entry **ent) {
  ...
  if (!buf->head) {
    for (int i = 0; i < STACK_BUFFER_NFILL; ++i) {
      entry = (stack_buffer_entry *)block_buffer_alloc(stack_entry_buffer);
      if (!entry)
        return -1;
