...
int cond_equal(dfa_state *entry, int cond) { return entry->cond == cond; }
...
void init_states(void) {
  for (int i = 1; i <= N_LR_STATES; ++i)
    states[i] = alloc_state(true);
  END = alloc_state(false);
}
...
int parse_loop(token_buffer *buf) {
  ...
  token *tk;
  dfa_state *entry;
  push_stack(&state_stack, 0);

  while (true) {
    state = stack_top(&state_stack);
    r = peek_token(buf, &tk);
    symbol = r == EOF ? EOF : tk->type;
    r = transfer(state, symbol, &entry);
    switch (r) {
    case 0:
      switch (entry->action) {
      case Shift:
        push_stack(&state_stack, entry->state);
        push_stack(&symbol_stack, symbol);
        get_token(buf, &tk);
        break;
