...
bool test_bitset(bitset * self, size_t pos)
{
  ...
  return (impl[pos / BITSET_BITS_OF_IMPL] & (1 << pos % BITSET_BITS_OF_IMPL));
}

void set_bitset(bitset * self, size_t pos)
{
  unsigned * impl = self->m_impl;
  impl[pos / BITSET_BITS_OF_IMPL] |= (1 << pos % BITSET_BITS_OF_IMPL);
}
...
void bitset_copy(bitset *self, bitset * other)
{
  if (self == other)
    return;
  memcpy (self->m_impl, other->m_impl, self->m_storage_size * BITSET_SIZEOF_IMPL);
}
