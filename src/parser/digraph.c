...
void digraph_init_trivial(digraph_struct * self)
{
...
void init_stack(digraph_struct * self)
{
	self->m_stack_top = 0;
}

void init_relation(digraph_struct * self)
{
	self->m_dimension = 0;
	...
	memcpy(self->m_unioned_data, self->m_data,
	       sizeof self->m_unioned_data);
}

void init_digraph(digraph_struct * self)
{
	init_stack(self);
	init_relation(self);
}

void push_stack(digraph_struct * self, int x)
{
	self->m_stack_array[self->m_stack_top++] = x;
}

int depth_of_stack(digraph_struct * self)
{
	return self->m_stack_top;
}

void pop_stack(digraph_struct * self)
{
...
int top_stack(digraph_struct * self)
{
	return self->m_stack_array[self->m_stack_top - 1];
}

void digraph_data_set(digraph_struct * self, int left_hand, int right_hand)
{
	self->m_unioned_data[left_hand] = self->m_unioned_data[right_hand];
}

void digraph_data_union(digraph_struct * self, int left_hand,
			int right_hand)
{
	self->m_unioned_data[left_hand] |=
	    self->m_unioned_data[right_hand];
}

void traverse(digraph_struct * self, int x)
{
	int depth, top;
	int *xid, *yid;
...
void digraph(digraph_struct * self)
{
