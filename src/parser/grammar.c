...
void init_clang(void) {
  ...
  DEF_ONEOF(type_spfr, DEF_KEYWORD("void"), DEF_KEYWORD("char"),
            DEF_KEYWORD("short"), DEF_KEYWORD("int"), DEF_KEYWORD("long"),
            DEF_KEYWORD("float"), DEF_KEYWORD("double"), DEF_KEYWORD("signed"),
            DEF_KEYWORD("unsigned"), struct_union_spfr, enum_spfr, id);

  DEF_ONEOF(type_qlfr, DEF_KEYWORD("const"), DEF_KEYWORD("volatile"));
  DEF_ONEOF(struct_or_union, DEF_KEYWORD("struct"), DEF_KEYWORD("union"));

  DEF_RULE(struct_union_spfr, struct_or_union, RULE_OPT, id, left_brace,
  ...
  DEF_RULE(dir_abs_dcltor, RULE_OPT, dir_abs_dcltor, left_bkt, RULE_OPT,
           const_expr, right_bkt);
  DEF_RULE(dir_abs_dcltor, RULE_OPT, dir_abs_dcltor, left_pare, RULE_OPT,
           param_type_list, right_pare);

  DEF_ONEOF(stmt, labeled_stmt, expr_stmt, block, selec_stmt, iter_stmt,
            jmp_stmt);

  DEF_RULE(labeled_stmt, id, colon, stmt);
  DEF_RULE(labeled_stmt, DEF_KEYWORD("case"), const_expr, colon, stmt);
  DEF_RULE(labeled_stmt, DEF_KEYWORD("default"), colon, stmt);

  DEF_RULE(expr_stmt, RULE_OPT, expr, semi);

  DEF_RULE(block, left_brace, RULE_OPT, dclr_list, RULE_OPT, stmt_list,
           right_brace);

  DEF_ONEMORE(stmt_list, stmt);

  DEF_RULE(selec_stmt, DEF_KEYWORD("if"), left_pare, expr, right_pare, stmt);

  DEF_RULE(selec_stmt, DEF_KEYWORD("if"), left_pare, expr, right_pare, stmt,
           DEF_KEYWORD("else"), stmt);

  DEF_RULE(selec_stmt, DEF_KEYWORD("switch"), left_pare, expr, right_pare,
           stmt);

  DEF_RULE(iter_stmt, _while, left_pare, expr, right_pare, stmt);

  DEF_RULE(iter_stmt, DEF_KEYWORD("do"), stmt, _while, left_pare, expr,
           right_pare, semi);

  DEF_RULE(iter_stmt, DEF_KEYWORD("for"), left_pare, RULE_OPT, expr, semi,
           RULE_OPT, expr, semi, RULE_OPT, expr, right_pare, stmt);
  ...
  DEF_ONEOF(assign_oper, equal, DEF_OPER("*="), DEF_OPER("/="), DEF_OPER("%="),
            DEF_OPER("+="), DEF_OPER("-="), DEF_OPER("<<="), DEF_OPER(">>="),
  ...
  DEF_ONEOF(primary, id, char_const, float_const, string_const, int_const);
