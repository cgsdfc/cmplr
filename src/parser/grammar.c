...
language lang_clang = {.name = "C programming language",
	.num_nonterm = 80,
	.num_terminal = 10,
	.num_keyword = 40,
	.num_operator = 50,
...
void init_clang(void)
{
	...
	DEF_RULE(func_def, RULE_OPT, dclr_spfr, dcltor, RULE_OPT,
		 dclr_list, block);
	...
	DEF_ONEOF(stcl_spfr, DEF_KEYWORD("auto"), DEF_KEYWORD("register"),
		  DEF_KEYWORD("static"), DEF_KEYWORD("extern"),
		  DEF_KEYWORD("typedef"));

	DEF_ONEOF(type_spfr, DEF_KEYWORD("void"), DEF_KEYWORD("char"),
		  DEF_KEYWORD("short"), DEF_KEYWORD("int"),
		  DEF_KEYWORD("long"), DEF_KEYWORD("float"),
		  DEF_KEYWORD("double"), DEF_KEYWORD("signed"),
		  DEF_KEYWORD("unsigned"), struct_union_spfr, enum_spfr,
		  id);

	DEF_ONEOF(type_qlfr, DEF_KEYWORD("const"),
		  DEF_KEYWORD("volatile"));
	DEF_ONEOF(struct_or_union, DEF_KEYWORD("struct"),
		  DEF_KEYWORD("union"));

	DEF_RULE(struct_union_spfr, struct_or_union, RULE_OPT, id,
	...
	DEF_RULE(init_dclr, dcltor, equal, init);
	...
	DEF_RULE(struct_dcltor, RULE_OPT, dcltor, colon, const_expr);

	DEF_RULE(enum_spfr, _enum, RULE_OPT, id, left_brace, enumtor_list,
		 right_brace);
	...
	DEF_RULE(dir_dcltor, dir_dcltor, left_bkt, RULE_OPT, const_expr,
		 right_bkt);
	DEF_RULE(dir_dcltor, dir_dcltor, left_pare, param_type_list,
		 right_pare);
	DEF_RULE(dir_dcltor, dir_dcltor, left_pare, RULE_OPT, id_list,
		 right_pare);
	...
	DEF_RULE(dir_abs_dcltor, RULE_OPT, dir_abs_dcltor, left_bkt,
		 RULE_OPT, const_expr, right_bkt);
	DEF_RULE(dir_abs_dcltor, RULE_OPT, dir_abs_dcltor, left_pare,
		 RULE_OPT, param_type_list, right_pare);

	DEF_ONEOF(stmt, labeled_stmt, expr_stmt, block, selec_stmt,
		  iter_stmt, jmp_stmt);

	DEF_RULE(labeled_stmt, id, colon, stmt);
	DEF_RULE(labeled_stmt, DEF_KEYWORD("case"), const_expr, colon,
		 stmt);
	DEF_RULE(labeled_stmt, DEF_KEYWORD("default"), colon, stmt);

	DEF_RULE(expr_stmt, RULE_OPT, expr, semi);

	DEF_RULE(block, left_brace, RULE_OPT, dclr_list, RULE_OPT,
		 stmt_list, right_brace);

	DEF_ONEMORE(stmt_list, stmt);

	DEF_RULE(selec_stmt, DEF_KEYWORD("if"), left_pare, expr,
		 right_pare, stmt);

	DEF_RULE(selec_stmt, DEF_KEYWORD("if"), left_pare, expr,
		 right_pare, stmt, DEF_KEYWORD("else"), stmt);

	DEF_RULE(selec_stmt, DEF_KEYWORD("switch"), left_pare, expr,
		 right_pare, stmt);

	DEF_RULE(iter_stmt, _while, left_pare, expr, right_pare, stmt);

	DEF_RULE(iter_stmt, DEF_KEYWORD("do"), stmt, _while, left_pare,
		 expr, right_pare, semi);

	DEF_RULE(iter_stmt, DEF_KEYWORD("for"), left_pare, RULE_OPT, expr,
		 semi, RULE_OPT, expr, semi, RULE_OPT, expr, right_pare,
		 stmt);

	DEF_RULE(jmp_stmt, DEF_KEYWORD("goto"), id, semi);

	DEF_RULE(jmp_stmt, DEF_KEYWORD("return"), RULE_OPT, expr, semi);

	DEF_RULE(jmp_stmt, DEF_KEYWORD("continue"), semi);

	DEF_RULE(jmp_stmt, DEF_KEYWORD("break"), semi);

	DEF_SEPMORE(expr, comma, assign_expr);
	DEF_RULE(assign_expr, cond_expr);
	DEF_RULE(assign_expr, unary_expr, assign_oper, assign_expr);
	DEF_ONEOF(assign_oper, equal, DEF_OPER("*="), DEF_OPER("/="),
		  DEF_OPER("%="), DEF_OPER("+="), DEF_OPER("-="),
		  DEF_OPER("<<="), DEF_OPER(">>="), DEF_OPER("&="),
		  DEF_OPER("^="), DEF_OPER("|="));

	DEF_RULE(cond_expr, lg_or_expr);
	DEF_RULE(cond_expr, lg_or_expr, question, expr, colon, cond_expr);
	...
	DEF_ONEOF(eq_oper, DEF_OPER("=="), DEF_OPER("!="));

	DEF_SEPMORE(rel_expr, rel_oper, shift_expr);

	DEF_ONEOF(rel_oper, DEF_OPER("<"), DEF_OPER(">"), DEF_OPER("<="),
		  DEF_OPER(">="));

	DEF_SEPMORE(shift_expr, shift_oper, add_expr);

	DEF_ONEOF(shift_oper, DEF_OPER("<<"), DEF_OPER(">>"));

	DEF_SEPMORE(add_expr, add_oper, mul_expr);

	DEF_ONEOF(add_oper, DEF_OPER("+"), DEF_OPER("-"));

	DEF_SEPMORE(mul_expr, mul_oper, cast_expr);

	DEF_ONEOF(mul_oper, DEF_OPER("*"), DEF_OPER("%"), DEF_OPER("/"));

	DEF_RULE(cast_expr, unary_expr);
	DEF_RULE(cast_expr, left_pare, type_name, right_pare, cast_expr);
	...
	DEF_ONEOF(unary_oper, DEF_OPER("&"), DEF_OPER("*"), DEF_OPER("+"),
		  DEF_OPER("-"), DEF_OPER("~"), DEF_OPER("!"));

	DEF_RULE(postfix, primary);
	DEF_RULE(postfix, postfix, left_bkt, expr, right_bkt);
	DEF_RULE(postfix, postfix, left_pare, RULE_OPT, arglist,
		 right_pare);
	...
	DEF_ONEOF(primary, id, char_const, float_const, string_const,
		  int_const);
