...
static char const *seek_line(scanner_input_buf *self,
  ...
  size_t cur_offset = (size_t)utillib_vector_at(&(self->line_offset), row - 1);
...
static char const *location_prefix(scanner_input_buf *self, int lv,
  ...
  return utillib_static_sprintf("%s/%s:%lu:%lu: %s ", absdir, self->filename,
                                UTILLIB_POS_ROW(pos), UTILLIB_POS_COL(pos),
...
/* access row */
size_t scanner_input_buf_row(scanner_input_buf *self) { return self->row; }

/* access col*/
size_t scanner_input_buf_col(scanner_input_buf *self) { return self->col; }
...
// arguments
utillib_error *scanner_input_buf_make_error(scanner_input_buf *self, int lv,
                                            char const *fmt, ...) {
  va_list ap;
  va_start(ap, fmt);
