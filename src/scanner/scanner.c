		  ...
		  scanner_match_entry_t const *match)
{
	...
	scanner_base_init(&self->sc_base, &self->sc_buf,
			  scanner_input_buf_getc, match);
}
...
static int try_match(scanner_base_t * self)
{
	...
	for (match = self->sc_match; match->scm_func != NULL; ++match) {
		...
		case SCANNER_CHAR:
			self->sc_text =
			    utillib_string_c_str(&self->sc_str);
...
char const *scanner_get_text(scanner_base_t * self)
{
	return self->sc_text;
}

int scanner_get_val(scanner_base_t * self)
{
	return self->sc_val;
}
...
int scanner_read_all(scanner_base_t * self, FILE * file)
{
	while (true) {
		...
		case SCANNER_EOF:
			return 0;
		case SCANNER_MATCHED:
			fprintf(file, "`%s':%d\n", scanner_get_text(self),
				scanner_get_val(self));
			break;
		case SCANNER_ERROR:
			err =
			    scanner_input_buf_make_error(self->sc_buf,
							 ERROR_LV_ERROR,
