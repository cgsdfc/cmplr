...
void init_float_literal(void)
{
  ...
  add_intermedia(TK_PERIOD, TK_FLOAT_FRACTION, CHAR_CLASS_DEC_DIGITS);
  add_intermedia(TK_INT_ZERO, TK_FLOAT_FRACTION, CHAR_CLASS_PERIOD);
  add_intermedia(TK_INT_DEC_BEGIN,TK_FLOAT_FRACTION,CHAR_CLASS_PERIOD);

  // entering exponential part e
  add_intermedia(TK_INT_ZERO,TK_FLOAT_EXPONENT,CHAR_CLASS_E);
  add_intermedia(TK_INT_DEC_BEGIN,TK_FLOAT_EXPONENT,CHAR_CLASS_E);
  ...
  add_intermedia(TK_FLOAT_EXPONENT,TK_FLOAT_SUFFIX_0, CHAR_CLASS_FLOAT_SUFFIX);

  // collect digits 
  add_selfloop(TK_FLOAT_FRACTION,CHAR_CLASS_DEC_DIGITS);
  add_selfloop(TK_FLOAT_EXPONENT,CHAR_CLASS_DEC_DIGITS);

  add_accepted (TK_FLOAT_EXPONENT, TK_FLOAT_END, CHAR_CLASS_SEPARATOR);
  add_accepted (TK_FLOAT_FRACTION, TK_FLOAT_END, CHAR_CLASS_SEPARATOR);
  add_accepted(TK_FLOAT_SUFFIX_0, TK_FLOAT_END, CHAR_CLASS_SEPARATOR);
